...
export default function ViewMarksList() {
  ...
  // ✅ Fetch active students (backend filters isDeleted: false) - Matches UpdateMarksList.jsx
  useEffect(() => {
    fetchStudents();
  }, []);
  ...
  const fetchStudents = async () => {
    try {
      ...
      console.log("🔍 Fetched students for marks view:", data); // ✅ Debug log
      if (response.ok) {
  ...
  // 🔍 Filter Students - Matches UpdateMarksList.jsx
  ...
  const handleAction = (action, id) => {
    ...
    } else if (action === "marksDetail") {
      navigate(`/marks-detail/${id}`); // ✅ Assuming marks detail page exists
    }
  };

  // ✅ FIXED: Photo Error Handler - Matches UpdateMarksList.jsx
  ...
  // Loading - Matches UpdateMarksList.jsx
  if (loading) {
    return (
  ...
  // Error - Matches UpdateMarksList.jsx
  if (error) {
    return (
      <div className="flex min-h-screen bg-gray-100">
        <Sidebar />
          ...
          <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-xl p-6 border border-gray-200">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <h2 className="text-2xl font-bold text-gray-800 border-b-4 border-red-600 inline-block pb-2">
                View Marks ({filteredStudents.length})
              </h2>
                    ...
                    >
                      ...
                      <td className="p-3 border text-center">
                        {student.studentPhoto ? (
                          <div className="relative w-10 h-10 mx-auto"> {/* ✅ Wrapper from UpdateMarksList */}
                            <img
                              src={`${API_BASE_URL}/uploads/${student.studentPhoto}`}
                              alt="Student Photo"
                              className="w-10 h-10 rounded-full object-cover absolute inset-0"
                              onError={(e) => handlePhotoError(e, student.studentPhoto)} // ✅ From UpdateMarksList
                      ...
                      <td className="p-3 border text-center">
                        ...
                        >
                          ...
                          <option value="marksDetail">📊 Marks Details</option>
                        </select>
                      </td>
                    </tr>
                  ))}

import React, { useState, useEffect, useMemo } from "react"; // ✅ NEW: useMemo for filtering
...
export default function UpdateMarksList() {
  ...
  // ✅ Fetch active students (backend filters isDeleted: false) - Matches DownloadAdmit.jsx
  useEffect(() => {
    fetchStudents();
  }, []);
  ...
  const fetchStudents = async () => {
    try {
      ...
      console.log("🔍 Fetched students for marks update:", data); // ✅ Debug log (shows {data: [...]})
      if (response.ok) {
  ...
  // 🔍 Filter Students - Matches DownloadAdmit.jsx
  ...
  const handleAction = (action, id) => {
    if (action === "view") {
      ...
      navigate(`/update-marks/${id}`); // ✅ FIXED: Consistent path (integrates with UpdateMarks.jsx)
    }
  };

  // ✅ FIXED: Photo Error Handler - Matches DownloadAdmit.jsx
  ...
  // Loading - Matches DownloadAdmit.jsx
  if (loading) {
    return (
  ...
  // Error - Matches DownloadAdmit.jsx
  if (error) {
    return (
      <div className="flex min-h-screen bg-gray-100">
        <Sidebar />
          ...
          <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-xl p-6 border border-gray-200">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
              <h2 className="text-2xl font-bold text-gray-800 border-b-4 border-red-600 inline-block pb-2">
                Update Marks ({filteredStudents.length})
              </h2>
                    ...
                    >
                      ...
                      <td className="p-3 border text-center">
                        {student.studentPhoto ? (
                          <div className="relative w-10 h-10 mx-auto"> {/* ✅ Wrapper from DownloadAdmit */}
                            <img
                              src={`${API_BASE_URL}/uploads/${student.studentPhoto}`}
                              alt="Student Photo"
                              className="w-10 h-10 rounded-full object-cover absolute inset-0"
                              onError={(e) => handlePhotoError(e, student.studentPhoto)} // ✅ From DownloadAdmit
                      ...
                      <td className="p-3 border text-center">
                        ...
                        >
                          ...
                          <option value="updateMarks">✏️ Update Marks</option>
                        </select>
                      </td>
                    </tr>
                  ))}
